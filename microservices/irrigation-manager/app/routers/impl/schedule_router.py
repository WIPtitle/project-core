...
class ScheduleRouter(RouterWrapper):
    ...
    def _define_routes(self):
        ...
        async def add_schedule(setup_id: int, body: dict, request: Request):
            ...
            if zone_id is None or day_of_week is None or not start_time_str or not end_time_str:
                return JSONResponse(status_code=400, content={"detail": "zone_id, day_of_week, start_time and end_time are required"})
            try:
            ...
            try:
                schedule = await self._service.add_schedule(
                    setup_id=setup_id,
                    zone_id=zone_id,
